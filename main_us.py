import torch
from data_us import StockDataset
...
class StockModelTGNN(nn.Module):
    def __init__(self, embed_dim, stock_feature_dim, hidden_dim, num_classes=2):
        ...
        # 股票特征用时序图神经网络提取
        self.stock_tgnn = TemporalGCN(stock_feature_dim, 64, num_layers=2)
    ...
    def forward(self, texts, stock_features, edge_index,all_text,edge_index1,label):
        ...
        # print(texts.shape,all_text.shape,edge_index.shape,edge_index1.shape)
        ...
        # print(edge_all)
        # print(len(stock_features),len(all_text))
        ...
        # text_repr = self.text_fc(text_repr)  # 降维到 hidden_dim
        ...
        # text_repr1 = self.text_fc1(text_repr1)
        ...
        # node_repr = self.stock_tgnn(stock_features[0], edge_index[0])  # (num_nodes, hidden_dim)
        # node_repr = self.dropout(node_repr)
        # windows = []
        # for i in range(stock_features.size(1) - self.window_size + 1):
        #     windows.append(stock_features[:, i:i + self.window_size, :])
        ...
        node_cat = torch.cat([stock_output1.squeeze(1),text_repr1],dim=0)
        edge_type = self.edge_attr(edge_type)
        node_gat = self.gat2(node_cat,edge_all[0],edge_attr=edge_type)
        node_gat = self.gat22(node_gat,edge_all[0],edge_attr=edge_type)
        node_stock = node_gat[:len(stock_output1.squeeze(1))]
        # node_stock
        # combined_repr = torch.cat((stock_output1.squeeze(1),text_repr, node_repr,node_stock), dim=1)  # (batch_size, hidden_dim * 2)
        combined_repr = torch.cat((stock_output1.squeeze(1),node_stock), dim=1) 
        # combined_repr = stock_output1.squeeze(1)
        # 二分类输出
        output = self.fc(combined_repr)  # (batch_size, num_classes)
        return output
    # def forward(self, texts, stock_features, edge_index):
    #     # 文本处理
    ...
    #     # 股票特征处理（时序图神经网络）
    #     stock_features = stock_features.to(device)  # (batch_size, num_nodes, feature_dim)
    #     print(stock_features.shape)
    #     all_stock_repr = []
    #     for t in range(stock_features.size(1)):  # 遍历每个时间步
    #         node_features = stock_features[:, t, :]  # 当前时间步的节点特征 (batch_size, num_nodes, feature_dim)
    #         node_repr = self.stock_tgnn(node_features.squeeze(0), edge_index)  # (num_nodes, hidden_dim)
    #         all_stock_repr.append(node_repr.mean(dim=0))  # 每个时间步取平均
    #     stock_repr = torch.stack(all_stock_repr, dim=0).mean(dim=0)  # (batch_size, hidden_dim)
    #     stock_repr = self.stock_fc(stock_repr)

    #     # 融合
    #     combined = torch.cat((text_repr, stock_repr), dim=1)  # (batch_size, hidden_dim * 2)
...
class StockModel(nn.Module):
    def __init__(self, embed_dim, stock_feature_dim, hidden_dim, num_classes=2):
        ...
        # 融合层
        self.fc = nn.Linear(288, num_classes)  # 两部分特征的融合输出
...
def custom_collate_fn(batch):
    ...
    edge_index = [item["edge_index"] for item in batch if item["edge_index"] is not None]
...
def main():
    ...
    dataset_val = StockDataset("/home/zhaokx/CMIN-Dataset-main/CMIN-US/news/preprocessed","/home/zhaokx/CMIN-Dataset-main/CMIN-US/price/processed",mode='val')
    # dataset_train = StockDataset("/home/zhaokx/CMIN-Dataset-main/acl18/tweet/preprocessed","/home/zhaokx/CMIN-Dataset-main/acl18/price/preprocessed",mode='train')
    # dataset_val = StockDataset("/home/zhaokx/CMIN-Dataset-main/acl18/tweet/preprocessed","/home/zhaokx/CMIN-Dataset-main/acl18/price/preprocessed",mode='val')
    batch_size = 1
    num_epochs = 50
    # dataset_size = len(dataset)
    # train_size = dataset_size // 2  # 训练集大小
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # outputs = model(texts.to(device), stock_features.to(device))
            # 计算损失
            ...
            # print(predicted,labels,batch['time'])
        ...
        with torch.no_grad():
            for batch in val_dataloader:
                ...
                # print("val:",predicted,labels,batch['time'])
